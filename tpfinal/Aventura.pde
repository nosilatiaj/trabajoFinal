class Aventura {
  ...
  int pantalla;
  Boton botonInicio, botonInicioJ, botonReinicio, botonReinicioCred, botonCreditos, botonIzq, botonDer;
  Dialogo dialogos;

  Aventura() {
    colorMode(HSB);
    botonInicioJ = new Boton ("Iniciar", width/2, 530, 170, 90);
    ...
    botonReinicioCred = new Boton ("Reiniciar", 100, 530, 170, 90);
  ...
  void botonEnPantalla () {
    if (pantalla == 0) {
      ...
      botonCreditos.dibujar(0, 255);
    } else if (pantalla == 6 && !juego.jugando) {
      botonInicioJ.dibujar(0, 255);
    } 
    if (pantalla == 29) {
      if (mouseX < width/2) {
  ...
  void caminoMouse() {
    if (pantalla == 0 && zonaCuadrada(121, 283, 200, 580 )) {
      pantalla=1;
    } else if (pantalla == 0 && zonaCuadrada(420, 584, 500, 580)) {
      pantalla = 82; //pantalla créditos
    } else if (pantalla == 36 && zonaCuadrada(121, 283, 200, 580)) {
      pantalla = 0; // vuelve de los créditos
    } else if (pantalla == 36 && zonaCuadrada(420, 584, 500, 580)) {
      pantalla = 82;
    }
    if (pantalla == 82 && zonaCuadrada(20, 183, 107, 572)) {
      pantalla = 0;
    ...
    if (pantalla == 6) {
      if (zonaCuadrada (267, 432, 356, 580)) {
  ...
  void creditos() {
    ...
    textSize(30);
    text("Mini aventura basada en el capitulo\n''ROCK BOTTOM'\n creado por\nCatalina Morel", width/2, 100 );
    text("&", width/2, 260);
    text("Eliana Iguain Iafrate", width/2, 290);
    text("Profesor", width/2, 340);
    text("Matias Jauregui Lorda", width/2, 380);
    text("Comision 2", width/2, 410);
    text("Tecnologia Multimedial", width/2, 460);
    text("Facultad de Artes", width/2, 500);
    text("2021", width/2, 530);
    botonReinicioCred.dibujar(0, 255);
    popStyle();
  }
}
